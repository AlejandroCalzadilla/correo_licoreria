...
public class Actualizar {
    ...
    /**
     * Ejecuta actualización de entidades basándose en los parámetros recibidos
     * (como string)
    ...
    private String actualizarRol(String[] params) {
        try {
            ...
            if (rol == null)
                return "Rol no encontrado con ID: " + id;
            if (params.length > 1)
                rol.setNombre(params[1]);
            if (params.length > 2)
                rol.setDescripcion(params[2]);
            if (params.length > 3)
                rol.setActivo(Boolean.parseBoolean(params[3]));
    ...
    private String actualizarUsuario(String[] params) {
        try {
            ...
            if (params.length > 1)
                usuario.setNombre(params[1]);
            if (params.length > 2)
                usuario.setCorreo(params[2]);
            if (params.length > 3)
                usuario.setClave(params[3]);
            if (params.length > 4)
                usuario.setEstado(params[4]);
            if (params.length > 5) {
                Rol rol = services.getRolService().findById(Long.parseLong(params[5])).orElse(null);
                if (rol == null)
                    return "Rol no encontrado con ID: " + params[5];
                usuario.setRol(rol);
            }
    ...
    private String actualizarVendedor(String[] params) {
        try {
            ...
            if (vendedor == null)
                return "Vendedor no encontrado con ID: " + id;
            if (params.length > 1)
                vendedor.setCi(params[1]);
            if (params.length > 2)
                vendedor.setNombre(params[2]);
            if (params.length > 3) {
    ...
    private String actualizarProducto(String[] params) {
        try {
            ...
            if (params.length > 2)
                producto.setNombre(params[2]);
            if (params.length > 3)
                producto.setPrecio(new BigDecimal(params[3]));
            if (params.length > 4)
                producto.setCodigo(params[4]);
            if (params.length > 5)
                producto.setDescripcion(params[5]);
            if (params.length > 6)
                producto.setMarca(params[6]);
    ...
    private String actualizarInventario(String[] params) {
        try {
            ...
            if (inventario == null)
                return "Inventario no encontrado con ID: " + id;
            if (params.length > 1) {
            ...
            if (params.length > 2)
                inventario.setCantidad(Integer.parseInt(params[2]));
            if (params.length > 3) {
    ...
    private String actualizarCompra(String[] params) {
        try {
            ...
            if (params.length > 2)
                compra.setDescripcion(params[2]);
            if (params.length > 3)
                compra.setEstado(params[3]);
    ...
    private String actualizarProveedor(String[] params) {
        try {
            ...
            if (params.length > 1)
                proveedor.setNombre(params[1]);
            if (params.length > 2)
                proveedor.setTelefono(params[2]);
            if (params.length > 3)
                proveedor.setDireccion(params[3]);
            if (params.length > 4)
                proveedor.setNit(params[4]);
            if (params.length > 5)
                proveedor.setCorreo(params[5]);
    ...
    private String actualizarCliente(String[] params) {
        try {
            ...
            if (params.length > 1)
                cliente.setCi(params[1]);
            if (params.length > 2)
                cliente.setNombre(params[2]);
            if (params.length > 3)
                cliente.setTelefono(params[3]);
            if (params.length > 4)
                cliente.setDireccion(params[4]);
            if (params.length > 5)
                cliente.setEstado(params[5].charAt(0));
            if (params.length > 6)
                cliente.setCreditoAprobado(Boolean.parseBoolean(params[6]));
            if (params.length > 7)
                cliente.setLimiteCredito(Double.parseDouble(params[7]));
            if (params.length > 8) {
                Usuario usuario = new Usuario();
    ...
    private String actualizarVenta(String[] params) {
        try {
            ...
            if (venta == null)
                return "Venta no encontrada con ID: " + id;
            if (params.length > 1) {
            ...
            if (params.length > 3)
                venta.setMontoTotal(new BigDecimal(params[3]));
            if (params.length > 4)
                venta.setSaldo(new BigDecimal(params[4]));
            if (params.length > 5)
                venta.setEstado(params[5]);
    ...
    private String actualizarCredito(String[] params) {
        try {
            ...
            if (params.length > 2)
                credito.setMontoTotal(new BigDecimal(params[2]));
            if (params.length > 3)
                credito.setSaldo(new BigDecimal(params[3]));
            if (params.length > 4)
                credito.setNumeroCuotas(params[4]);
            if (params.length > 5)
                credito.setEstado(params[5]);
